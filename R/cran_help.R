...
cran_help_cliques <- function(packages = NULL) {
  ...
  df_links <- data.frame(
    from = paste0(cpl$from_pkg, ":", cpl$from_Rd),
  ...
  isolated_help <- lapply(graph_decomposed, function(x) {
  ...
  if (!is.null(packages)) {
    valid_clique <- vapply(split(df, df$clique), function(x) {
      any(packages %in% x$from_pkg | packages %in% x$to_pkg)
    }, logical(1L))
