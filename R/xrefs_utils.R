...
split_anchor <- function(links, count = TRUE) {
  links_targets <- strcapture_m("([[:alnum:].]*{2,})?[:=]?(.*)",
    x = links[, "Anchor"],
    proto = c("to_pkg", "to_target")
  )
  link_w_targets <- cbind(links, links_targets)
...
targets2files <- function(links, alias) {
  ...
  # No duplicated alias
  da <- dup_alias(alias)
  browser(expr = is.null(dim(da)))
  dalias <- unique(alias[, "Target"])
  ...
  links <- cbind(links, tmp = seq_len(NROW(links)))
  ...
  table_x <- split(
    links_w_files$Source.x[diff_paths_x],
  ...
  links_w_files <- sort_by(links_w_files, links_w_files[, c("from_pkg", "from_Rd", "to_target", "to_Rd")])
